...
if ($loggedin === true) {
	if (!empty($_POST['buy']) && $_SESSION['shop_session'] == $_POST['session']) {
		...
		} else echo '<font color="red" size="4">You need more guild points, this offer cost '.$buy['guild_points'].' guild points.</font>';
		//var_dump($buy);
...
<h1>Guild Shop Offers</h1>
<?php
if ($loggedin === true) {
	if (!empty($_POST['buy']) && $_SESSION['shop_session'] == $_POST['session']) {
		if ($user_znote_data['guild_points'] >= $buy['guild_points']) {
			?><td>You have <?php echo (int)($user_znote_data['guild_points'] - $buy['guild_points']); ?> guild points.</td><?php
		} else {
			?><td>You have <?php echo $user_znote_data['guild_points']; ?> guild points.</td><?php
		}
	} else {
		?><td>You have <?php echo $user_znote_data['guild_points']; ?> guild points.</td><?php
	}
		...
		<?php foreach ($category_items as $key => $offers): ?>
			<tr class="special">
				...
				<?php if ($config['guild_shop']['showImage']):?>
					<td><img src="http://<?php echo $config['guild_shop']['imageServer']; ?>/<?php echo $offers['itemid']; ?>.<?php echo $config['guild_shop']['imageType']; ?>" alt="img"></td>
				<?php endif; ?>
...
} else echo '<h1>Buy Guild Points system disabled.</h1><p>Sorry, this functionality is disabled.</p>';
